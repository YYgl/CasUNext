...
class Conv(nn.Module):
    def __init__(self, dim):
        ...
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, stride=1, groups=dim,
...
class CasUNeXt(nn.Module):
    def __init__(self,
                 in_channels: int = 1,
                 num_classes: int = 2,
                 bilinear: bool = True,
                 base_c: int = 32):
        super(CasUNeXt, self).__init__()
...
if __name__ == '__main__':
    model = CasUNeXt(in_channels=3, num_classes=2, base_c=32).to('cuda')
