...
class FetalDataset(Dataset):
    def __init__(self, root: str, train: bool, coarse: bool, transforms=None):
        super(FetalDataset, self).__init__()
        self.flag = "train" if train else "test"
        self.pattern = "coarse" if coarse else "fine"
        self.path = self.pattern + "_" + self.flag
        data_root = os.path.join(root, self.path)
